              ...
              int fortran_particle_comp, int nghost )
{
    ...
    for (int lev = 0; lev < nlev; lev++)
    {
      ...
      amrex::Print() << "PARTICLE BA " << ParticleBoxArray(lev) << std::endl;

      amrex::Print() << "FLAGS BA " << flags->boxArray() << std::endl;
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
       {
        ...
        for (ParConstIter pti(*this, lev); pti.isValid(); ++pti) {
            ...
            std::cout << "MY PROC / BOX " <<  ParallelDescriptor::MyProc() << " " <<  box  << std::endl;
#endif

            const Box& bx  = pti.tilebox(); // I need a box without ghosts

            std::cout << "MY PROC / BX " <<  ParallelDescriptor::MyProc() << " " <<  box  << std::endl;
            ...
            if ((*flags)[pti].getType(bx) != FabType::covered )
            {
                ...
                std::cout << "VOLFRAC BX  " << ((*volfrac)[pti]).box() << std::endl;
                std::cout << "FLAGS BX    " << ((*flags)[pti]).box() << std::endl;
                std::cout << "FAB   BX    " << fab.box() << std::endl;
