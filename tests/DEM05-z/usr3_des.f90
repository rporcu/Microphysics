...
! Calculate the particle-wall rebound angle.
            NP = LC+31
            VEL_XY(1) = sqrt(des_vel_new(NP,1)**2 + des_vel_new(NP,2)**2)
            RBND_ANGLE(1) = atan(VEL_XY(1)/des_vel_new(NP,3))*180.0/PI

! Calculate the particle-wall rebound angle.
            NP = LC
            VEL_XY(2) = sqrt(des_vel_new(NP,1)**2 + des_vel_new(NP,2)**2)
            RBND_ANGLE(2) = atan(VEL_XY(2)/des_vel_new(NP,3))*180.0/PI
...
! Calculate the particle-wall restitution coefficient.
            NP=LC+31
            VEL_XY(1) = sqrt(des_vel_new(NP,1)**2 + des_vel_new(NP,2)**2)
            RST_COEFF(1) =  VEL_XY(1)/INIT_VEL_T(NP)
! Calculate the particle-wall restitution coefficient.
            NP=LC
            VEL_XY(2) = sqrt(des_vel_new(NP,1)**2 + des_vel_new(NP,2)**2)
            RST_COEFF(2) =  VEL_XY(2)/INIT_VEL_T(NP)
! Write the results to a file.
...
! Calculate particle-particle angular velocity
            NP=LC+31
            ROT_XY(1) = sqrt(omega_new(NP,1)**2 + omega_new(NP,2)**2)

! Calculate particle-wall angular velocity
            NP=LC
            ROT_XY(2) = sqrt(omega_new(NP,1)**2 + omega_new(NP,2)**2)

! Write the results to a file.
            WRITE(UDF_UNIT,"(3(3x,F11.4))") INIT_ANGLE(NP), ROT_XY(1:2)
         ENDDO
         CLOSE(UDF_UNIT)
      ENDIF
