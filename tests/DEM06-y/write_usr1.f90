...
subroutine WRITE_DES_Out(lTime, np, particles)
   ...
   WRITE(lUNIT,1000) lTime, RK4_POS(2), particles(1) % pos(2),        &
        ABS_ERR(RK4_POS(2), particles(1) % pos(2))
   ...
   OPEN(UNIT=lUNIT, FILE='POST_VEL.dat',                            &
        POSITION="APPEND", STATUS='OLD')
   ! Write the results to file.
   WRITE(lUNIT,1000) lTime, RK4_VEL(2), particles(1) % vel(2),        &
        ABS_ERR(RK4_VEL(2), particles(1) % vel(2))
   ! Close the output file.
   CLOSE(lUNIT)
...
!  1000 FORMAT(3x,F15.6,5X,F15.6,3x,F15.6,3x,F15.2)
