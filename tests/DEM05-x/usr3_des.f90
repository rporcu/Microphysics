...
subroutine usr3_des ( np, particles )
   ...
   do LC = 1, 31
      ...
      RBND_ANGLE(1) = atan(VEL_YZ(1)/particles(p) % vel(1))*180.0/PI

      ! Calculate the particle-wall rebound angle.
      P = LC
      VEL_YZ(2) = sqrt(particles(p) % vel(2)**2 + particles(p) % vel(3)**2)
      RBND_ANGLE(2) = atan(VEL_YZ(2)/particles(p) % vel(1))*180.0/PI
   ...
   do LC = 1, 31
      ...
      P=LC+31
      ROT_YZ(1) = sqrt(particles(p) % omega(2)**2 + particles(p) % omega(3)**2)

      ! Calculate particle-wall angular velocity
      P=LC
      ROT_YZ(2) = sqrt(particles(p) % omega(2)**2 + particles(p) % omega(3)**2)
