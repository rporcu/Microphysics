...
subroutine write_usr1(l, np, time, dt, particles, xlength, ylength, zlength )
   ...
   implicit none
      ...
      select case(L)
         case(1); call write_des_out(time, np, particles, zlength)
      end select
...
!......................................................................!
      ...
      use amrex_fort_module, only: c_real => amrex_real
      use usr,               only: gz1, gz2, gy1, gy2, rk4_v4
...
! file unit for heat transfer data
   ...
   rel_diff = (gz1 - particles(1) % pos(3)) / gz1
   rel_diff = dabs(rel_diff) * 100.d0

   ! Write the results to a file.
   write(uPos1,"(3x,F15.8,5X,3(3x,F15.8))") lTime, gz1, particles(1) % pos(3), rel_diff

   rel_diff = (gz2 - particles(2) % pos(3)) / gz2
   rel_diff = dabs(rel_diff) * 100.d0

   write(uPos2,"(3x,F15.8,5X,3(3x,F15.8))") lTime, gz2, particles(2) % pos(3), rel_diff
