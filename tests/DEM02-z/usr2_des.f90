...
subroutine usr2_des( np, particles )
   ...
   if ( particles(1) % vel(3) < 0.0) then
      if(yVELO > 0.0) then
         if(BOUNCE_COUNT < MAX_BOUNCE) then
            BOUNCE_COUNT = BOUNCE_COUNT + 1
            MAX_HEIGHT(BOUNCE_COUNT) = max(yPOSO, particles(1) % vel(3) )
         ENDIF
      ENDIF
   ENDIF

   yVELO = particles(1) % vel(3)
   yPOSO = particles(1) % pos(3)
...
! ! Check if particle reached the peak of the bounce.
!       IF(DES_VEL_new(1,3) < 0.0) THEN
!          IF(yVELO > 0.0) THEN
!             IF(BOUNCE_COUNT < MAX_BOUNCE) THEN
!                BOUNCE_COUNT = BOUNCE_COUNT + 1
!                MAX_HEIGHT(BOUNCE_COUNT) = max(yPOSO, DES_POS_NEW(1,3))
!             ENDIF
!          ENDIF
!       ENDIF

!       yVELO = DES_VEL_NEW(1,3)
!       yPOSO = DES_POS_NEW(1,3)
