...
subroutine WRITE_DES_Out(lTime, np, particles)
   ...
   Pos_aErr = abs(lPos_Y - particles(1) % pos(3) )
   ...
   Vel_aErr = abs( lVel_Y - particles(1) % vel(3) )
   ...
   write(uPos,"(F15.8,5x,I1,5X,F15.8,3(3x,F15.8))") lTime, &
        lStage, lPos_Y, particles(1) % pos(3), Pos_rErr, Pos_aErr
   close(uPos)


   write(uVel,"(F15.8,5x,I1,5X,F15.8,3(3x,F15.8))")lTime, &
        lStage, lVel_Y, particles(1) % vel(3), Vel_rErr, Vel_aErr
   close(uVel)
...
! ! Calculate the absolute and absolute relative errors.
!       Pos_aErr = abs(lPos_Y - DES_POS_new(1,1))
!        if(lPos_Y > 0.0d0) then
!           Pos_rErr = Pos_aErr/lPos_Y*100
!        else
!           Pos_rErr = Pos_aErr*100
...
!        Vel_aErr = abs(lVel_Y - DES_VEL_new(1,1))
!        if(lVel_Y > 0.0d0) then
!           Vel_rErr = Vel_aErr/lVel_Y*100
!        else
!           Vel_rErr = Vel_aErr*100
...
! ! Write the results to a file.
!        WRITE(uPos,"(F15.8,5x,I1,5X,F15.8,3(3x,F15.8))") lTime, &
!           lStage, lPos_Y,DES_POS_new(1,1),Pos_rErr, Pos_aErr
!       CLOSE(uPos)


!       WRITE(uVel,"(F15.8,5x,I1,5X,F15.8,3(3x,F15.8))")lTime, &
!          lStage, lVel_Y,DES_VEL_new(1,1),Vel_rErr, Vel_aErr
!       CLOSE(uVel)
