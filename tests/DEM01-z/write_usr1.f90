...
! Calculate the absolute and absolute relative errors.
      Pos_aErr = abs(lPos_Y - DES_POS_new(1,3))
       ...
       Vel_aErr = abs(lVel_Y - DES_VEL_new(1,3))
...
! Write the results to a file.
       WRITE(uPos,"(F15.8,5x,I1,5X,F15.8,3(3x,F15.8))") lTime, &
          lStage, lPos_Y,DES_POS_new(1,3),Pos_rErr, Pos_aErr
      CLOSE(uPos)


      WRITE(uVel,"(F15.8,5x,I1,5X,F15.8,3(3x,F15.8))")lTime, &
         lStage, lVel_Y,DES_VEL_new(1,3),Vel_rErr, Vel_aErr
      CLOSE(uVel)
