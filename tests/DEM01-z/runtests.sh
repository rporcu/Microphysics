...
for DES_ETA in 0.9 0.8 0.7 0.6; do
    ...
    time -p ${MFIX} inputs \
       "dem.spring_const_pp=${DES_KN}        dem.spring_const_pw=${DES_KN}" \
       "dem.restitution_coeff_pp=${DES_ETA}  dem.restitution_coeff_pw=${DES_ETA}"

    write_data "POST_POS.dat" ${DES_KN} ${DES_ETA}  3
    write_data "POST_VEL.dat" ${DES_KN} ${DES_ETA} 11
