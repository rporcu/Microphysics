                            ...
                            Real& /*coupling_timing*/)
{
    ...
    if (need_divtau()) {
      diffusion_op->ComputeDivTau(get_divtau(), get_vel_g_old(), get_ep_g(), get_T_g_old(), advect_enthalpy, GetVecOfConstPtrs(eb_flow_vel));
    }

    {
      ...
      const bool update_flux = (solve_species   && (l_explicit_diff || constraint));
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        for (MFIter mfi(*ld.vel_g,TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
          ...
          ParallelFor(bx, [nspecies_g,epg,rho_o,rho_n,X_gk_o,dXdt_o,l_dt,X_gk_n,
              X_rhs_o,l_explicit_diff,div_J_o]
            AMREX_GPU_DEVICE (int i, int j, int k) noexcept
          {
            const Real epg_loc = epg(i,j,k);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
          ...
          for (MFIter mfi(*ld.vel_g,TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            ...
            ParallelFor(bx, [nspecies_g,epg,rho_o,rho_n,X_gk_o,dXdt_o,l_dt,X_gk_n,
                X_RHS_o,l_explicit_diff,div_J_n]
              AMREX_GPU_DEVICE (int i, int j, int k) noexcept
            {
              const Real epg_loc = epg(i,j,k);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    if (!l_explicit_diff) {
      ...
      // Diffuse velocity
      diffusion_op->diffuse_velocity(get_vel_g(), get_ep_g(), get_T_g(), advect_enthalpy, l_dt, GetVecOfConstPtrs(eb_flow_vel));
