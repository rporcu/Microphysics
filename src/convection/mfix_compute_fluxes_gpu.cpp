                                  ...
                                  const FArrayBox& ep_w_mac)
{
  ...
  size_t numberOfBlocks = Gpu::Device::warp_size*Gpu::Device::numMultiProcessors();

  amrex::launch_global<<<numberOfBlocks,threadsPerBlock,0,Gpu::gpuStream()>>>(
//    [slopes_comp,state_comp,dom_low,dom_high,bct_ilo,bct_ihi,bct_jlo,bct_jhi,
//     bct_klo,bct_khi,bc_types,state,x_slopes,y_slopes,z_slopes,u,v,w,fx,fy,fz,
//     ubx_npoints,ubx_lo,ubx_len,vbx_npoints,vbx_lo,vbx_len,wbx_npoints,wbx_lo,
//     wbx_len,npoints,ncomp]
    [=]
    AMREX_GPU_DEVICE () noexcept
  {
    ...
    for(int idx = index; idx < npoints; idx += stride)
    {
      if(idx < ubx_npoints)
      {
                                     ...
                                     const EBCellFlagFab& flags)
{
  ...
  amrex::launch_global<<<numberOfBlocks,threadsPerBlock,0,Gpu::gpuStream()>>>(
//    [my_huge,slopes_comp,state_comp,dom_low,dom_high,bct_ilo,bct_ihi,bct_jlo,
//     bct_jhi,bct_klo,bct_khi,bc_types,areafrac_x,areafrac_y,areafrac_z,fcx_fab,
//     fcy_fab,fcz_fab,ccc_fab,x_slopes,y_slopes,z_slopes,state,u,v,w,sx,sy,sz,
//     fx,fy,fz,ubx_npoints,ubx_lo,ubx_len,vbx_npoints,vbx_lo,vbx_len,wbx_npoints,
//     wbx_lo,wbx_len,npoints,ncomp]
    [=]
    AMREX_GPU_DEVICE () noexcept
  {
    int index = threadIdx.x + blockDim.x*blockIdx.x;
