    ...
    for (int n = 0; n < ncomp; n++)
    {
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            ...
            for (int ii = -1; ii <= 1; ii++)
            {
                ...
                {
                    int r = i+ii; int s = j+jj; int t = k+kk;
                    dUdt(r,s,t,n+icomp) += soln_hat(i,j,k,n) + slopes_eb[0] * (ccent(r,s,t,0)-cent_hat(i,j,k,0))
#if (AMREX_SPACEDIM == 3)
    ...
    { // STRT:SUM OF FINAL DUDT
      for (int n = 0; n < ncomp; n++)
      {
        ...
        for (int i = bx.smallEnd(0); i <= domain.bigEnd(0); i++)
        {
            ...
            sum2 += vfrac(i,j,k)*dUdt(i,j,k,n+icomp);
        }
