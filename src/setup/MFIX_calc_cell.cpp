//vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv!
//                                                                      !
//  Subroutine: calc_cell_ic                                            !
//  Purpose: calculate the i, j or k cell index for IC regions.         !
//                                                                      !
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
#include <iostream>
#include <iomanip>
#include <fstream>
#include <sstream>
#include <AMReX_Math.H>
#include <MFIX_calc_cell.H>
#include <AMReX_AmrParGDB.H>
// #include <cmath>

using namespace amrex;

void calc_cell_ic(const Real dx,
                  const Real dy,
                  const Real dz,
                  const amrex::Real* lo,
                  const amrex::Real* hi,
                  const amrex::Real* plo,
                  int& i_w,
                  int& i_e,
                  int& j_s,
                  int& j_n,
                  int& k_b,
                  int& k_t)
{
  i_w = static_cast<int>(amrex::Math::floor((lo[0]-plo[0])/dx - .5));
  i_e = static_cast<int>(amrex::Math::floor((hi[0]-plo[0])/dx - .5));

  j_s = static_cast<int>(amrex::Math::floor((lo[1]-plo[1])/dy - .5));
  j_n = static_cast<int>(amrex::Math::floor((hi[1]-plo[1])/dy - .5));

  k_b = static_cast<int>(amrex::Math::floor((lo[2]-plo[2])/dz + .5));
  k_t = static_cast<int>(amrex::Math::floor((hi[2]-plo[2])/dz + .5));

}