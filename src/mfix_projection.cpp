				  ...
				  Real dt, Real& prev_dt, Real time, Real normg)
{
    ...
    amrex::Print() << " UG at T=0 HAS NANs = " << u_g[lev] -> contains_nan () << "\n";
...
#pragma omp parallel
    ...
    amrex::Print() << " UG HAS NANs = " << u_g[lev] -> contains_nan () << "\n";
    amrex::Print() << " VG HAS NANs = " << v_g[lev] -> contains_nan () << "\n";
    amrex::Print() << " WG HAS NANs = " << w_g[lev] -> contains_nan () << "\n";
...
#pragma omp parallel 
	...
	amrex::Print() << " UGO HAS NANs = " << u_go[lev] -> contains_nan () << "\n";
	amrex::Print() << " UG HAS NANs = "  << u_g[lev] -> contains_nan () << "\n";
	int dir = 1;
	compute_intermediate_velocity ( BL_TO_FORTRAN_BOX(bx),  
					...
					&dt, &dir);
	amrex::Print() << " FGDSU HAS NANs = "  << f_gds_u[lev] -> contains_nan () << "\n";
	amrex::Print() << " DRAGU HAS NANs = "  << drag_u[lev] -> contains_nan () << "\n";
	amrex::Print() << " UG* HAS NANs = "  << u_g[lev] -> contains_nan () << "\n";
    }

    amrex::Print() << " UGRADU_X HAS NANs = " << ugradu_x[lev] -> contains_nan () << "\n";
    amrex::Print() << " DIVTAU_X HAS NANs = " << divtau_x[lev] -> contains_nan () << "\n";
...
#pragma omp parallel
    ...
    // This hack is like removing nullspace from singular matrix
    offset = ( trD_g[lev] -> sum () )  / geom[lev].Domain ().numPts ();

    amrex::Print () << "OFFSET is " << offset << "\n";

    amrex::Print () << "PROB SIZE is " << geom[lev].ProbSize()  << "\n";
	
    //trD_g[lev] -> plus ( -offset, 1 );
    
    amrex::Print () << "SUM(rhs) is " << ( trD_g[lev] -> sum () ) << "\n";
				      ...
				      Vector< std::unique_ptr<MultiFab> >& rhs )
{
    ...
    // Multigrid inputs
    Vector<int>                         bc(2*AMREX_SPACEDIM, -1); // Neumann boundaries
    bool                                nodal = false;
