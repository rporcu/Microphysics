               ...
               IntVect& Nrep)
{
  ...
  if (FLUID::solve)
  {
    ...
    for (int lev = 0, nlevs=finestLevel()+1; lev < nlevs; ++lev)
    {
      ...
      MultiFab mf_vel;
      VisMF::Read(mf_vel, amrex::MultiFabFileFullPrefix(lev, restart_file,
            level_prefix, "u_g"));

      MultiFab mf_gp;
      VisMF::Read(mf_gp, amrex::MultiFabFileFullPrefix(lev, restart_file,
            level_prefix, "gpx"));
  ...
  if (FLUID::solve)
  {
    for (int lev = 0; lev <= finestLevel(); lev++)
    {
      ...
      ep_go.FillBoundary(geom[lev].periodicity());
