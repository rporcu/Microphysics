...
FluidPhase::FluidPhase()
  : viscosity_model(VISCOSITYMODEL::Invalid)
  ...
  if ((ntypes > 0) && (amrex::toLower(names[0]).compare("none") != 0)) {
    ...
    if (solve_enthalpy) {
      ...
      if (solve_species) {
        ...
        specific_heat_model = species.SpecificHeatModel;
    ...
    p_therm_defined = pp.query("thermodynamic_pressure", thermodynamic_pressure);

    if (constraint_type == ConstraintType::IncompressibleFluid && p_therm_defined) {

      amrex::Warning("When the incompressible fluid constraint is selected, "
          "the fluid thermodynamic pressure input will be ignored");
    }

    if (constraint_type == ConstraintType::IdealGasClosedSystem && (!p_therm_defined)) {
    ...
    // Allocate parameters
    parameters = new FluidParms(T_ref);

    parameters->m_mu_g = mu_g0;
    parameters->m_k_g = k_g0;
    parameters->m_nspecies = nspecies;
    ...
    if (MW_gk0.size() > 0) {
      ...
      parameters->m_h_MW_gk = MW_gk0.data();
      parameters->m_d_MW_gk = d_MW_gk0.data();
    }

    parameters->m_D_g = D_g0;

    if (cp_gk0.size() > 0) {
      d_cp_gk0.resize(cp_gk0.size());
      Gpu::copyAsync(Gpu::hostToDevice, cp_gk0.begin(), cp_gk0.end(), d_cp_gk0.begin());
      parameters->m_h_cp_gk = cp_gk0.data();
      parameters->m_d_cp_gk = d_cp_gk0.data();
    }

    if (H_fk0.size() > 0) {
      d_H_fk0.resize(H_fk0.size());
      Gpu::copyAsync(Gpu::hostToDevice, H_fk0.begin(), H_fk0.end(), d_H_fk0.begin());
      parameters->m_h_H_fk = H_fk0.data();
      parameters->m_d_H_fk = d_H_fk0.data();
    }

    parameters->m_nreactions = reactions.nreactions;
