...
Species::Initialize ()
{
  ...
  if (pp.contains("solve")) {
    ...
    pp.getarr("solve", names);

    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(names.size() > 0,
    ...
    if(solve) {
      ...
      for (int n(0); n < nspecies; n++) {
        std::string name = "species." + names[n];
        amrex::ParmParse ppSpecies(name.c_str());

        if (!ppSpecies.query("molecular_weight", MW_k0[n])) {

          if (amrex::ParallelDescriptor::IOProcessor()) {
            std::string message = "Input not provided. Assuming MW_" + names[n] + " = 0";
        ...
        if (SpecificHeatModel == SPECIFICHEATMODEL::Constant) {
          ...
          for (int n(0); n < nspecies; n++) {
            ...
            if(!ppSpecies.query("enthalpy_of_formation", H_fk0[n])) {
              ...
              if (amrex::ParallelDescriptor::IOProcessor()) {
                std::string message = "Input not provided. Assuming Hf_" + names[n] + " = 0";
                amrex::Warning(message.c_str());
              }
            }
          }
