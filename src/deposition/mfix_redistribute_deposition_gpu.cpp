...
//  MultiFab overflow_aux(mf_to_redistribute.boxArray(),
//                        mf_to_redistribute.DistributionMap(),
//                        mf_to_redistribute.nComp(),
//                        mf_to_redistribute.nGrow(),
//                        MFInfo(),
//                        mf_to_redistribute.Factory());
//  overflow_aux.setVal(0.);
...
//      Array4<Real> const& auxiliary = overflow_aux.array(mfi);
        ...
        AMREX_GPU_DEVICE (int i, int j, int k) noexcept
      {
        if(flags(i,j,k).isSingleValued() and ep_s(i,j,k) > max_eps)
        {
          ...
          for(int n(0); n < ncomp; n++) {
//                Real redist_val = mf_redist(i,j,k,n) + auxiliary(i,j,k,n);
