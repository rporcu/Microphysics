     ...
     if ( (*flags_fab)[mfi].getType(amrex::grow(bx,0)) != FabType::covered &&
       ...
       if(sm_size <= Gpu::Device::sharedMemPerBlock()) {
            ...
            mask_box,bx_lo,bx_hi] AMREX_GPU_DEVICE () noexcept
         {
           ...
           get_index idx(blockDim.x+2,blockDim.y+2,blockDim.z+2,1);
           ...
           if((i <= bx_hi[0]) and (j <= bx_hi[1]) and (k <= bx_hi[2]))
           {
             ...
             for (int kk(0); kk < 3; kk++) {
               ...
               eps_sm[idx(t_x,t_y,t_z)] = ep_s(g_x,g_y,g_z);
               vfrac_sm[idx(t_x,t_y,t_z)] = vfrac(g_x,g_y,g_z);
               flags_sm[idx(t_x,t_y,t_z)] = flags(g_x,g_y,g_z);
             }

             __syncthreads();

             if(flags_sm[idx(tid_x,tid_y,tid_z)].isSingleValued() and
                eps_sm[idx(tid_x,tid_y,tid_z)] > max_eps)
             {
               ...
               for(int kk(-1); kk <= 1; kk++) {
                 if((ii != 0 or jj != 0 or kk != 0 ) and
                    flags_sm[idx(tid_x,tid_y,tid_z)].isConnected({ii,jj,kk}) and
                    mask_box.contains(IntVect(i+ii,j+jj,k+kk)) and
                    ((not flags_sm[idx(tid_x+ii,tid_y+jj,tid_z+kk)].isSingleValued()) or
                     (eps_sm[idx(tid_x+ii,tid_y+jj,tid_z+kk)] <= max_eps)))
                 {
                   const Real vfrac_val = vfrac_sm[idx(tid_x+ii,tid_y+jj,tid_z+kk)];
                   const Real eps_val = eps_sm[idx(tid_x+ii,tid_y+jj,tid_z+kk)];
               ...
               const Real eps_val = eps_sm[idx(tid_x,tid_y,tid_z)];
               ...
               for(int n(0); n < ncomp; n++) {
                 ...
                 Real overflow =
                   duplicate(i,j,k,n)*scale*vfrac_sm[idx(tid_x,tid_y,tid_z)]/sum_vfrac;
