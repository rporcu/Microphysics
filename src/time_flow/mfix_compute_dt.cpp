...
#pragma omp parallel reduction(max:cfl_max) if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(*vel_g[lev], TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
      ...
      if (flags.getType(bx) != FabType::covered)
      {
        ...
        {
          if (not flags_fab(i,j,k).isCovered())
          {  
            Real acc[3];
  ...
  // Protect against cfl_max very small This may happen, for example, when the
  // initial velocity field is zero for an inviscid flow with no external
  // forcing
  Real eps = numeric_limits<Real>::epsilon();
  if ( nstep > 1 && cfl_max <= eps ) dt_new = 0.5 * old_dt;

  // Don't let the timestep grow by more than 1% per step.  unless the previous
  // time step was unduly shrunk to match plot_per_exact
  ...
  // dt_new is the step calculated with a cfl contraint; dt is the value set by
  // fixed_dt When the test was on dt > dt_new, there were cases where they were
  // effectively equal but (dt > dt_new) was being set to true due to precision
  // issues.
  Real ope(1.0 + 1.e-8);
