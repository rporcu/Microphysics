...
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
      for (MFIter mfi(*trac_in[lev], false); mfi.isValid(); ++mfi) {

        FArrayBox& scal_fab = (*trac_in[lev])[mfi];
      ...
      trac_in[lev]->FillBoundary(m_geom[lev].periodicity());

      EB_set_covered(*trac_in[lev], 0, trac_in[lev]->nComp(),
          trac_in[lev]->nGrow(), mfix::covered_val);
    }
  }
}
