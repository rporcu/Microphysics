...
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
    for (MFIter mfi(*(ro_g_in[lev]), false); mfi.isValid(); ++mfi) {

      FArrayBox& scal_fab = (*ro_g_in[lev])[mfi];
    ...
    ro_g_in[lev]->FillBoundary(m_geom[lev].periodicity());

    EB_set_covered(*ro_g_in[lev], 0, ro_g_in[lev]->nComp(),
        ro_g_in[lev]->nGrow(), mfix::covered_val);
  }
}
