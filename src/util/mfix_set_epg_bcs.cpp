...
void
MFIXBoundaryConditions::set_epg_bcs (const Vector< MultiFab* >& epg_in,
                                     const int dir_bc)
{
  BL_PROFILE("MFIXBoundaryConditions::set_epg_bcs()");

  const int nlev = epg_in.size();

  for (int lev = 0; lev < nlev; lev++)
  {
    // Set all values outside the domain to covered_val just to avoid use of undefined
    epg_in[lev]->setDomainBndry(mfix::covered_val, m_geom[lev]);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(*epg_in[lev], false); mfi.isValid(); ++mfi) {
      ...
      FArrayBox& epg_fab = (*epg_in[lev])[mfi];
    ...
    EB_set_covered(*epg_in[lev], 0, epg_in[lev]->nComp(), epg_in[lev]->nGrow(),
        mfix::covered_val);

    // Do this after as well as before to pick up terms that got updated in the call above
    epg_in[lev]->FillBoundary(m_geom[lev].periodicity());
  }
}
