...
//
// Set the BCs for species mass fractions only
//
void
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
    for (MFIter mfi(*X_gk_in[lev], false); mfi.isValid(); ++mfi) {

      FArrayBox& X_gk_fab = (*X_gk_in[lev])[mfi];
    ...
    X_gk_in[lev]->FillBoundary(m_geom[lev].periodicity());

    EB_set_covered(*X_gk_in[lev], 0, X_gk_in[lev]->nComp(),
        X_gk_in[lev]->nGrow(), mfix::covered_val);
  }
}
