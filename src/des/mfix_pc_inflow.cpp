                ...
                EBFArrayBoxFactory* factory)
{
  ...
  for (int bcv(0); bcv < m_boundary_conditions.bc().size(); ++bcv) {
    ...
    for (MFIter mfi = MakeMFIter(lev); mfi.isValid(); ++mfi) {
      ...
      const Box bx_int = tile_box&(bc_bx);
      ...
      Array4<Real const> const& barea = (t != FabType::singlevalued) ? Array4<Real const>{} :
                                         factory->getBndryArea().const_array(mfi);
      Array4<Real const> const& bcent = (t != FabType::singlevalued) ? Array4<Real const>{} :
                                         factory->getBndryCent()[mfi].const_array();
      Array4<Real const> const& afrac = (t != FabType::singlevalued) ? Array4<Real const>{} :
                                         factory->getAreaFrac()[dir]->const_array(mfi);

      Real fab_area(0.);

      IntVect bblo(bx_int.loVect());
      IntVect bbhi(bx_int.loVect());

      if (t == FabType::regular ) {

        fab_area = dx2*static_cast<Real>(bx_int.numPts());

      } else { // (t == FabType::singlevalued )
        ...
        reduce_op.eval(bx_int, reduce_data, [flagsfab, barea, bnorm, bcent,
          ...
          dir, dir_lohi, afrac, is_mi]
        AMREX_GPU_DEVICE (int i, int j, int k) -> ReduceTuple
        {
          Real area(0.0);

          if (is_mi) {
            if (flagsfab(i,j,k).isRegular()) {
              area = 1.0;
      ...
      for(int lcs(0); lcs < m_boundary_conditions.bc(bcv).solids.size(); lcs++) {
        ...
        if(m_boundary_conditions.bc(bcv).solids[lcs].volfrac < tolerance) { continue; }
        ...
        // bc area 'available' to solids
        const Real act_area = bc_solid.volfrac*fab_area;
        // fraction of bc area on this fab
        const Real fab_afrac = fab_area/bc_area;

        // Scale volflow by the fraction of the area contained in this fab.
        const Real fab_volflow = bc_solid.volflow*fab_afrac;

        const Real volflow = (bc_solid.velmag < tolerance) ? fab_volflow :
            act_area*bc_solid.velmag;

        const Real velmag = amrex::max(bc_solid.velmag, volflow/act_area);
        RealVect velvec(bc_solid.velocity[0],bc_solid.velocity[1],bc_solid.velocity[2]);
        ...
        amrex::ParallelForRNG(pcount, [pstruct, ptile_data, NextID, MyProc, old_np, bbx,
          p_real,p_int, phase, state, is_dem, is_pic, statwt, plo, dx, idx, rbbx,
          regular_fab, is_mi, dir_lohi, dir, shift_dir, shift_lohi, eff_rad_scale,
          rand_x, rand_y, rand_z, velmag, velvec,
          ...
          while ( !valid ) {
            ...
            if ( bbx.contains(IntVect(i,j,k))) {
              if ( regular_fab ) {
                ...
                valid = true;
                if(has_normal) { // has_normal is false for mass inflows
            ...
            if (valid) {
              ...
              if (is_mi) { // mass inflow (pic only)
                ...
                p.pos(dir) = shift_lohi*eff_rad +
                  static_cast<Real>(ijk[dir]+dir_lohi)*dx[dir];

                p_real[SoArealData::velx][ip] = velvec[0];
                p_real[SoArealData::vely][ip] = velvec[1];
                p_real[SoArealData::velz][ip] = velvec[2];
