      ...
      for (MFIXParIter pti(*pc, lev); pti.isValid(); ++pti)
      {
        ...
        if (flags.getType(amrex::grow(bx,0)) != FabType::covered)
        {
          ...
          else // FAB not all regular
          {
            ...
            amrex::ParallelFor(np,
              [pstruct,vel_array,gp_array,flags_array,gp0_dev,plo,dxi,ccent_fab]
              AMREX_GPU_DEVICE (int ip) noexcept
              {
                amrex::Real velfp[3];
                amrex::Real gradp[3];
                      ...
                      for(int jj(0); jj<2; jj++){
                        for(int ii(0); ii<2; ii++){
                          if( flags_array(i,j,k).isConnected(ioff+ii,joff+jj,koff+kk)) {
                            ...
                            gradp[0] +=  gp_array(ilo+ii,jlo+jj,klo+kk,0)*weight;
                            gradp[1] +=  gp_array(ilo+ii,jlo+jj,klo+kk,1)*weight;
                            gradp[2] +=  gp_array(ilo+ii,jlo+jj,klo+kk,2)*weight;
                          }
                        }
                      }
                    }
