...
contains
  ...
  subroutine des_time_march( np, slo, shi, ulo, uhi, vlo, vhi, wlo, whi, &
       ...
       & pstate, pphase, des_radius,  pvol, pmass, omoi, des_pos_new,    &
       & des_vel_new, omega_new, des_acc_old, rot_acc_old, drag_fc, time,&
       & dt, dx, dy, dz, xlength, ylength, zlength, nstep) &
    ...
    real(c_real), intent(inout) :: pvol(np)
    real(c_real), intent(inout) :: pmass(np)
    real(c_real), intent(inout) :: des_radius(np)
    real(c_real), intent(inout) :: omoi(np)

    real(c_real), intent(inout) :: des_pos_new(np,3)
    real(c_real), intent(inout) :: des_vel_new(np,3)

    real(c_real), intent(inout) :: des_acc_old(np,3)
    real(c_real), intent(inout) :: rot_acc_old(np,3)
    real(c_real), intent(inout) :: drag_fc(np,3)
    real(c_real)                :: fc(np,3)
    real(c_real)                :: tow(np,3)

    real(c_real), intent(inout) :: omega_new(np,3)

    integer(c_int), intent(inout) :: pstate(np)
    integer(c_int), intent(inout) :: pphase(np)

    integer(c_int)                :: pairs(6*np,2)
    integer(c_int)                :: pair_count
...
1100 FORMAT(/'Time: ',g12.5,3x,'DT: ',g12.5,3x,'DEM NITs: ',A)
    ...
    IF(des_continuum_coupled) THEN
       IF(DES_EXPLICITLY_COUPLED) THEN
          call drag_gs_des(slo, shi, ulo, uhi, vlo, vhi, wlo, whi, np, &
               ...
               gradPg, pstate, pvol, des_pos_new, &
               des_vel_new, fc, des_radius,  pphase, dx, dy, dz)
       ENDIF
       call calc_pg_grad(slo, shi, lo, hi, np, &
            p_g, gradPg,  pstate, des_pos_new, &
    ...
    DO NN = 1, FACTOR
       ...
       call calc_drag_des(slo,shi, ulo, uhi, vlo, vhi, wlo, whi,np,&
            ep_g,u_g,v_g,w_g,ro_g,mu_g, gradPg, pstate, fc,&
            drag_fc,pvol, des_pos_new,des_vel_new,&
            des_radius,pphase,dx, dy, dz)

       ! Call user functions.
       IF(call_usr) call USR1_DES
       ! Update position and velocities
       call CFNEWVALUES(np, pstate, pmass, omoi, &
       ...
       IF(call_usr) call USR2_DES(np, des_pos_new, des_vel_new, omega_new)
    ...
    IF(call_usr) call USR3_DES(np, des_pos_new, des_vel_new, omega_new)
...
9000   FORMAT('    NITs/SEC = ',A)
  ...
  CONTAINS
    ...
    SUBROUTINE CALC_COLLISIONS(np, pairs, pair_count, &
      ...
      real(c_real), intent(in   ) :: des_pos_new(np,3)
      real(c_real), intent(in   ) :: des_radius(np)
      integer     , intent(in   ) :: pstate(np)
      ...
      DO LL = 1, np-1
         ...
         IF(NONEXISTENT==PSTATE(LL)) CYCLE
         pos = DES_POS_NEW(LL,:)
         rad = DES_RADIUS(LL)

         DO I = LL+1, np
            IF(NONEXISTENT==PSTATE(I)) CYCLE
