...
contains
   ...
   subroutine calc_force_dem(phase, radius, pos, vel, omega, state, fc, tow)
      ...
      real(c_real), intent(in)    :: radius(:), pos(:,:), vel(:,:), omega(:,:)
...
8550           format('distance between particles is zero:',2(2x,i10))
            ...
            call cfrelvel(ll, ii, v_rel_trans_norm, vrel_t,            &
            ...
            dist_cl = q2 * ( dist_mag  + ( radius(ll)**2 - radius(ii)**2) / dist_mag )
   ...
   contains
      ...
      subroutine print_excess_overlap
         ...
         if(overlap_n > flag_overlap*radius(ll) .or.                  &
              overlap_n > flag_overlap*radius(ii)) then

            write(err_msg,1000) trim(ival(ll)), trim(ival(ii)), s_time,    &
                 radius(ll), radius(ii), overlap_n
...
1000     format('warning: excessive overplay detected between ',          &
   ...
   !                                                                      !
   !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
   subroutine calc_force_dem_aos( particles, fc, tow )
...
1000     format('warning: excessive overplay detected between ',          &
   ...
   end subroutine calc_force_dem_aos
