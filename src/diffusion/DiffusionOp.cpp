...
// Implicit solve for scalar diffusion
//
void DiffusionOp::diffuse_ep_g (Vector<std::unique_ptr<LevelData>>& leveldata,
                                const Vector<std::unique_ptr<MultiFab>>& ep_ro_in,
                                const Vector<Real> mu_s, Real dt)
{
  BL_PROFILE("DiffusionOp::diffuse_ep_g");
  ...
  MultiFab& ep_g0 = leveldata[0]->ep_g;

  int ntrac = ep_g0.nComp();
  ...
  for(int lev = 0; lev <= finest_level; lev++)
  {
    ...
    // Zero these out just to have a clean start because they have
    // 3 components (due to re-use with velocity solve)
    phi[lev]->setVal(0.0);
    rhs[lev]->setVal(0.0);

    // Set the right hand side to equal rhs
    MultiFab::Copy((*rhs[lev]),ep_g, 0, 0, ntrac, 0);
    ...
    MultiFab::Copy(*phi[lev],ep_g, 0, 0, ntrac, 1);
    scal_matrix->setLevelBC(lev, GetVecOfConstPtrs(phi)[lev]);
  }
  ...
  for(int lev = 0; lev <= finest_level; lev++)
  {
    ...
    phi[lev]->FillBoundary(geom[lev].periodicity());
    MultiFab::Copy(ep_g, *phi[lev], 0, 0, ntrac, 1);
  }
}
