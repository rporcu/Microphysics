...
void mfix::mfix_calc_volume_fraction (Real& sum_vol)
{
  ...
  if (DEM::solve)
  {
    ...
    for (int lev = 0; lev < nlev; lev++)
    {
      ...
      if (lev == 0 and OnSameGrids) {
        ...
        mf_pointer[lev] = new MultiFab(ba_crse,
                                       pc->ParticleDistributionMap(lev),
                                       ep_g.nComp(), 1);
      }
    ...
    for (int lev = 1; lev < nlev; lev++)
      mf_pointer[0]->copy(*mf_pointer[lev], 0, 0, (m_leveldata[lev]->ep_g).nComp(),
                          src_nghost, dest_nghost, gm.periodicity(),
                          FabArrayBase::ADD);
    ...
    if (mf_pointer[0] != &ep_g0)
      ep_g0.copy(*mf_pointer[0],0,0,ep_g0.nComp());
    ...
    if(mfix::m_deposition_diffusion_coeff > 0.)
      mfix_diffuse_ep_g(m_leveldata, mfix::m_deposition_diffusion_coeff);

    for (int lev = 0; lev < nlev; lev++) {
      MultiFab& ep_g = m_leveldata[lev]->ep_g;

      // Now define this mf = (1 - particle_vol)
      ep_g.mult(-1., ep_g.nGrow());
      ep_g.plus( 1., ep_g.nGrow());
      ...
      EB_set_covered(ep_g, 1.0);
    }

    // HACK -- we really should average down (ep_g * volfrac) not ep_g.
    for (int lev(nlev-1); lev > 0; lev--) {
      amrex::EB_average_down(m_leveldata[lev]->ep_g, m_leveldata[lev-1]->ep_g,
  ...
  MultiFab& ep_g = m_leveldata[lev]->ep_g;
  sum_vol = volWgtSum(lev, ep_g, comp);
}
