      ...
      for (int lev = 0; lev <= finest_level; lev++)
      {
        ...
        for (MFIter mfi(*ld.vel_g,TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
                 ...
                 (flagfab.getType(amrex::grow(bx,1)) != FabType::regular) )
            {
                ...
                {
                    Real delta_vel = vel_redist(i,j,k,n+icomp) - vel_orig(i,j,k,n);
                    gradp(i,j,k,n+icomp) -= delta_vel * ep_g(i,j,k) / (l_dt * sig(i,j,k));
                });
                ...
                AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                {
                    vel_redist(i,j,k,n+icomp) = vel_orig(i,j,k,n);
                });
            }
        }
