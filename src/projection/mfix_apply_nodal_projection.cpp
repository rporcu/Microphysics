                                   ...
                                   bool proj_2 )
{
  ...
  // If we have dropped the dt substantially for whatever reason, use
  // a different form of the approximate projection that projects (U^*-U^n + dt
  // Gp) rather than (U^* + dt Gp)

  if (a_time > 0 and a_dt < .1*prev_dt)
  ...
  if (proj_for_small_dt)
  {
    ...
    for(int lev = 0; lev <= finest_level; lev++)
      MultiFab::Saxpy(*vel_g[lev], -1.0, *vel_go[lev], 0, 0, 3, vel_g[lev]->nGrow());
  }
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    // that no-slip walls are treated exactly like slip walls -- Note that this
    // routine is essential to impose the correct inflow bc's on the product
    // ep * vel
    for (MFIter mfi((*epu[lev]), TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
      // Why are we using this instead of simply multiplying vel and ep with
      // their BCs in place already?
      set_vec_bcs(lev, (*epu[lev])[mfi], geom[lev].Domain());
    }

    epu[lev]->FillBoundary(geom[lev].periodicity());

    // We set these to zero because if the values in the covered cells are
    // undefined, even though they are multiplied by zero in the divu
    // computation, we can still get NaNs
  ...
  nodal_projector.reset(new NodalProjector(GetVecOfPtrs(vel_g),
                                           GetVecOfConstPtrs(sigma), geom, info));
  ...
  {
    for(int lev = 0; lev <= finest_level; lev++)
      MultiFab::Saxpy(*vel_g[lev], 1., *vel_go[lev], 0, 0, 3, vel_g[lev]->nGrow());
  }
