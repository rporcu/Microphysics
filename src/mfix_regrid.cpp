...
mfix_level::RegridArrays (int lev, BoxArray& new_grids, DistributionMapping& new_dmap)
{
    ...
    // X-axis gas velocity
    std::unique_ptr<MultiFab> u_g_new(new MultiFab(x_edge_ba,new_dmap,1,u_g[lev]->nGrow()));
    u_g_new->copy(*u_g[lev]);
    u_g[lev] = std::move(u_g_new);

    std::unique_ptr<MultiFab> u_go_new(new MultiFab(x_edge_ba,new_dmap,1,u_go[lev]->nGrow()));
    u_go_new->copy(*u_go[lev]);
    u_go[lev] = std::move(u_go_new);

    std::unique_ptr<MultiFab> u_gt_new(new MultiFab(x_edge_ba,new_dmap,1,u_gt[lev]->nGrow()));
    ...
    std::unique_ptr<MultiFab> ropX_new(new MultiFab(x_edge_ba,new_dmap,1,ropX[lev]->nGrow()));
    ...
    // Y-axis gas velocity
    std::unique_ptr<MultiFab> v_g_new(new MultiFab(y_edge_ba,new_dmap,1,v_g[lev]->nGrow()));
    v_g_new->copy(*v_g[lev]);
    v_g[lev] = std::move(v_g_new);

    std::unique_ptr<MultiFab> v_go_new(new MultiFab(y_edge_ba,new_dmap,1,v_go[lev]->nGrow()));
    v_go_new->copy(*v_go[lev]);
    v_go[lev] = std::move(v_go_new);

    std::unique_ptr<MultiFab> v_gt_new(new MultiFab(y_edge_ba,new_dmap,1,v_gt[lev]->nGrow()));
    ...
    std::unique_ptr<MultiFab> ropY_new(new MultiFab(y_edge_ba,new_dmap,1,ropY[lev]->nGrow()));
    ...
    // Z-axis gas velocity
    std::unique_ptr<MultiFab> w_g_new(new MultiFab(z_edge_ba,new_dmap,1,w_g[lev]->nGrow()));
    w_g_new->copy(*w_g[lev]);
    w_g[lev] = std::move(w_g_new);

    std::unique_ptr<MultiFab> w_go_new(new MultiFab(z_edge_ba,new_dmap,1,w_go[lev]->nGrow()));
    w_go_new->copy(*w_go[lev]);
    w_go[lev] = std::move(w_go_new);

    std::unique_ptr<MultiFab> w_gt_new(new MultiFab(z_edge_ba,new_dmap,1,w_gt[lev]->nGrow()));
    w_gt_new->copy(*w_gt[lev]);
    w_gt[lev] = std::move(w_gt_new);

    std::unique_ptr<MultiFab> d_t_new(new MultiFab(z_edge_ba,new_dmap,1,d_t[lev]->nGrow()));
    ...
    std::unique_ptr<MultiFab> ropZ_new(new MultiFab(z_edge_ba,new_dmap,1,ropZ[lev]->nGrow()));
