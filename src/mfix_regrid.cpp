...
mfix_level::RegridArrays (int lev, BoxArray& new_grids, DistributionMapping& new_dmap)
{
    ...
    ep_g_new->copy(*ep_g[lev],0,0,1,ng,ng,geom[lev].periodicity());
    ep_g[lev] = std::move(ep_g_new);

    // Old void fraction
    ng = ep_go[lev]->nGrow();
    std::unique_ptr<MultiFab> ep_go_new(new MultiFab(new_grids,new_dmap,1,ep_go[lev]->nGrow()));
    ep_go_new->copy(*ep_go[lev],0,0,1,ng,ng,geom[lev].periodicity());
    ...
    p_g_new->copy(*p_g[lev],0,0,1,ng,ng,geom[lev].periodicity());
    ...
    rop_go_new->copy(*rop_go[lev],0,0,1,ng,ng,geom[lev].periodicity());
    ...
    lambda_g_new->copy(*lambda_g[lev],0,0,1,ng,ng,geom[lev].periodicity());
    lambda_g[lev] = std::move(lambda_g_new);

    // Trace(D)
    ng = trD_g[lev]->nGrow();
    std::unique_ptr<MultiFab> trD_g_new(new MultiFab(new_grids,new_dmap,1,trD_g[lev]->nGrow()));
    trD_g_new->copy(*trD_g[lev],0,0,1,ng,ng,geom[lev].periodicity());
