...
mfix_level::RegridArrays (int lev)
{
    ...
    // Old void fraction
    ng = ep_go[lev]->nGrow();
    std::unique_ptr<MultiFab> ep_go_new(new MultiFab(grids[lev],dmap[lev],1,ep_go[lev]->nGrow()));
    ...
    // Gas density
    ng = ro_g[lev]->nGrow();
    std::unique_ptr<MultiFab> ro_g_new(new MultiFab(grids[lev],dmap[lev],1,ro_g[lev]->nGrow()));
    ro_g_new->setVal(0.0);
    ...
    // Old gas density
    ng = ro_go[lev]->nGrow();
    std::unique_ptr<MultiFab> ro_go_new(new MultiFab(grids[lev],dmap[lev],1,ro_go[lev]->nGrow()));
    ...
    // Gas bulk density
    ng = rop_g[lev]->nGrow();
    std::unique_ptr<MultiFab> rop_g_new(new MultiFab(grids[lev],dmap[lev],1,rop_g[lev]->nGrow()));
    ...
    ng = rop_go[lev]->nGrow();
    std::unique_ptr<MultiFab> rop_go_new(new MultiFab(grids[lev],dmap[lev],1,rop_go[lev]->nGrow()));
    ...
    if (nodal_pressure)
    {
       ...
       ng = p_g[lev]->nGrow();
       std::unique_ptr<MultiFab> p_g_new(new MultiFab(nd_grids,dmap[lev],1,ng));
       p_g_new->setVal(0.0);
       p_g_new->copy(*p_g[lev],0,0,1,0,ng);
       p_g[lev] = std::move(p_g_new);

       ng = p_go[lev]->nGrow();
       std::unique_ptr<MultiFab> p_go_new(new MultiFab(nd_grids,dmap[lev],1,ng));
       p_go_new->setVal(0.0);
       p_go_new->copy(*p_go[lev],0,0,1,0,ng);
       p_go[lev] = std::move(p_go_new);

       ng = p0_g[lev]->nGrow();
       std::unique_ptr<MultiFab> p0_g_new(new MultiFab(nd_grids,dmap[lev],1,ng));
       p0_g_new->setVal(0.0);
       p0_g_new->copy(*p0_g[lev],0,0,1,0,ng);
       p0_g[lev] = std::move(p0_g_new);

       ng = pp_g[lev]->nGrow();
       std::unique_ptr<MultiFab> pp_g_new(new MultiFab(nd_grids,dmap[lev],1,ng));
       pp_g_new->FillBoundary(p0_periodicity);
       pp_g_new->copy(*pp_g[lev],0,0,1,0,ng);
       pp_g[lev] = std::move(pp_g_new);

       std::unique_ptr<MultiFab> diveu_new(new MultiFab(nd_grids,dmap[lev],1,diveu[lev]->nGrow()));
       diveu[lev] = std::move(diveu_new);
       diveu[lev]->setVal(0.);

       std::unique_ptr<MultiFab> phi_new(new MultiFab(nd_grids,dmap[lev],1,phi[lev]->nGrow()));
       ...
       ng = p_g[lev]->nGrow();
       std::unique_ptr<MultiFab> p_g_new(new MultiFab(grids[lev],dmap[lev],1,ng));
       p_g_new->setVal(0.0);
       p_g_new->copy(*p_g[lev],0,0,1,0,ng);
       p_g[lev] = std::move(p_g_new);

       ng = p_go[lev]->nGrow();
       std::unique_ptr<MultiFab> p_go_new(new MultiFab(grids[lev],dmap[lev],1,ng));
       p_go_new->setVal(0.0);
       p_go_new->copy(*p_go[lev],0,0,1,0,ng);
       p_go[lev] = std::move(p_go_new);

       ng = p0_g[lev]->nGrow();
       std::unique_ptr<MultiFab> p0_g_new(new MultiFab(grids[lev],dmap[lev],1,ng));
       p0_g_new->setVal(0.0);
       p0_g_new->copy(*p0_g[lev],0,0,1,0,ng);
       p0_g[lev] = std::move(p0_g_new);

       ng = pp_g[lev]->nGrow();
       std::unique_ptr<MultiFab> pp_g_new(new MultiFab(grids[lev],dmap[lev],1,ng));
       pp_g[lev]->setVal(0.0);
       pp_g_new->copy(*pp_g[lev],0,0,1,0,ng);
       pp_g[lev] = std::move(pp_g_new);

       std::unique_ptr<MultiFab> phi_new(new MultiFab(grids[lev],dmap[lev],1,phi[lev]->nGrow(),
                                                      MFInfo(), *ebfactory[lev]));
       phi[lev] = std::move(phi_new);
       phi[lev]->setVal(0.);

       std::unique_ptr<MultiFab> diveu_new(new MultiFab(grids[lev],dmap[lev],1,diveu[lev]->nGrow(),
                                                      MFInfo(), *ebfactory[lev]));
       diveu[lev] = std::move(diveu_new);
       diveu[lev]->setVal(0.);
    ...
    // Molecular viscosity
    ng = mu_g[lev]->nGrow();
    std::unique_ptr<MultiFab> mu_g_new(new MultiFab(grids[lev],dmap[lev],1,ng));
    mu_g_new->setVal(0.0);
    ...
    std::unique_ptr<MultiFab> lambda_g_new(new MultiFab(grids[lev],dmap[lev],1,ng));
    ...
    // Trace(D)
    ng = trD_g[lev]->nGrow();
    std::unique_ptr<MultiFab> trD_g_new(new MultiFab(grids[lev],dmap[lev],1,ng));
