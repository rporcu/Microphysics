				...
				const Real c = 1.0 )
{
   ...
   for ( int lev=0; lev <= m_amrcore->finestLevel() ; ++lev )
   {
      ...
      areafrac =    (*m_ebfactory)[lev] -> getAreaFrac() ;
      volfrac  = &( (*m_ebfactory)[lev] -> getVolFrac() );
...
#pragma omp parallel
      ...
      for (MFIter mfi(*m_diveu[lev],true); mfi.isValid(); ++mfi)
      {
         ...
         {
            m_diveu[lev] -> setVal( 1.2345e300, bx, 0, 1);
         }
         else
         {
            ...
            {
               compute_mac_diveu_eb ( BL_TO_FORTRAN_BOX(bx),
                                      BL_TO_FORTRAN_ANYD((*m_diveu[lev])[mfi]),
                                      (*ep[lev])[mfi].dataPtr (),
                                      BL_TO_FORTRAN_ANYD((*u[lev])[mfi]),
                                      BL_TO_FORTRAN_ANYD((*v[lev])[mfi]),
...
MacProjection::solve_for_phi () 
{
   ...
   //                                 info);//
   MLEBABecLap             matrix( m_amrcore -> Geom(),
                                   m_amrcore -> boxArray(),
                                   m_amrcore -> DistributionMap(),
                                   info,
                                   amrex::GetVecOfConstPtrs(*(m_ebfactory)) );
