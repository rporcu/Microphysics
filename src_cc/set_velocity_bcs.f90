      ...
      do k = ulo(3), uhi(3)
         do j = ulo(2), uhi(2)
            ...
            case ( minf_)
               ...
               vel(ulo(1):domlo(1)-1,j,k,2) =  0.d0
               vel(ulo(1):domlo(1)-1,j,k,3) =  0.d0

            case ( nsw_) 

               vel(ulo(1):domlo(1)-1,j,k,1) = 0.d0
               vel(ulo(1):domlo(1)-1,j,k,2) = 0.d0
               vel(ulo(1):domlo(1)-1,j,k,3) = 0.d0

            case ( fsw_)
               
               vel(ulo(1):domlo(1)-1,j,k,1) = 0.0d0
               vel(ulo(1):domlo(1)-1,j,k,2) = vel(domlo(1),j,k,2)
               vel(ulo(1):domlo(1)-1,j,k,3) = vel(domlo(1),j,k,3)
            ...
            if (extrap_dir_bcs .gt. 0) then
               ...
               case ( minf_, nsw_, fsw_, psw_) 
                     vel(domlo(1)-1,j,k,1:3) = 2.d0*vel(domlo(1)-1,j,k,1:3) - vel(domlo(1),j,k,1:3)
               end select
            end if
      ...
      do k = ulo(3),uhi(3)
         do j = ulo(2),uhi(2)
            ...
            case ( minf_ )
               ...
               vel(domhi(1)+1:uhi(1),j,k,2) = 0.0d0
               vel(domhi(1)+1:uhi(1),j,k,3) = 0.0d0

            case ( nsw_ ) 

               vel(domhi(1)+1:uhi(1),j,k,1) = 0.0d0
               vel(domhi(1)+1:uhi(1),j,k,2) = 0.d0
               vel(domhi(1)+1:uhi(1),j,k,3) = 0.d0
            ...
            if (extrap_dir_bcs .gt. 0) then
               select case (bct_ihi(j,k,1))
               case ( minf_, nsw_, fsw_, psw_) 
                     vel(domhi(1)+1,j,k,1:3) = 2.d0*vel(domhi(1)+1,j,k,1:3) - vel(domhi(1),j,k,1:3)
               end select
            end if
      ...
      do k = ulo(3), uhi(3)
         do i = ulo(1), uhi(1)
            ...
            case ( minf_ )
               ...
               vel(i,ulo(2):domlo(2)-1,k,1) = 0.0d0
               vel(i,ulo(2):domlo(2)-1,k,2) = bc_v_g(bcv)
               vel(i,ulo(2):domlo(2)-1,k,3) = 0.0d0

            case ( nsw_ )

               vel(i,ulo(2):domlo(2)-1,k,1) = 0.d0
               vel(i,ulo(2):domlo(2)-1,k,2) = 0.0d0
               vel(i,ulo(2):domlo(2)-1,k,3) = 0.d0
            ...
            if (extrap_dir_bcs .gt. 0) then
               select case (bct_jlo(i,k,1))
               case ( minf_, nsw_, fsw_, psw_) 
                     vel(i,domlo(2)-1,k,1:3) = 2.d0*vel(i,domlo(2)-1,k,1:3) - vel(i,domlo(2),k,1:3)
               end select
            end if
      ...
      do k = ulo(3), uhi(3)
         do i = ulo(1), uhi(1)
            ...
            case ( minf_) 
               ...
               vel(i,domhi(2)+1:uhi(2),k,1) = 0.0d0
               vel(i,domhi(2)+1:uhi(2),k,2) = bc_v_g(bcv)
               vel(i,domhi(2)+1:uhi(2),k,3) = 0.0d0

            case ( nsw_) 

               vel(i,domhi(2)+1:uhi(2),k,1) = 0.d0
               vel(i,domhi(2)+1:uhi(2),k,2) = 0.0d0
               vel(i,domhi(2)+1:uhi(2),k,3) = 0.d0
            ...
            if (extrap_dir_bcs .gt. 0) then
               select case (bct_jhi(i,k,1))
               case ( minf_, nsw_, fsw_, psw_) 
                     vel(i,domhi(2)+1,k,1:3) = 2.d0*vel(i,domhi(2)+1,k,1:3) - vel(i,domhi(2),k,1:3)
               end select
            end if
      ...
      do j = ulo(2), uhi(2)
         do i = ulo(1), uhi(1)
            ...
            case ( minf_ )
               ...
               vel(i,j,ulo(3):domlo(3)-1,1) = 0.0d0
               vel(i,j,ulo(3):domlo(3)-1,2) = 0.0d0
               vel(i,j,ulo(3):domlo(3)-1,3) = bc_w_g(bcv)

            case ( nsw_ )

               vel(i,j,ulo(3):domlo(3)-1,1) = 0.d0
               vel(i,j,ulo(3):domlo(3)-1,2) = 0.d0
               vel(i,j,ulo(3):domlo(3)-1,3) = 0.0d0
            ...
            if (extrap_dir_bcs .gt. 0) then
               select case (bct_klo(i,j,1))
               case ( minf_, nsw_, fsw_, psw_) 
                     vel(i,j,domlo(3)-1,1:3) = 2.d0*vel(i,j,domlo(3)-1,1:3) - vel(i,j,domlo(3),1:3)
               end select
            end if
      ...
      do j = ulo(2), uhi(2)
         do i = ulo(1), uhi(1)
            ...
            case ( minf_ ) 
               ...
               vel(i,j,domhi(3)+1:uhi(3),1) = 0.0d0
               vel(i,j,domhi(3)+1:uhi(3),2) = 0.0d0
               vel(i,j,domhi(3)+1:uhi(3),3) = bc_w_g(bcv)

            case ( nsw_ ) 

               vel(i,j,domhi(3)+1:uhi(3),1) = 0.d0
               vel(i,j,domhi(3)+1:uhi(3),2) = 0.d0
               vel(i,j,domhi(3)+1:uhi(3),3) = 0.0d0
            ...
            if (extrap_dir_bcs .gt. 0) then
               select case (bct_khi(i,j,1))
               case ( minf_, nsw_, fsw_, psw_) 
                     vel(i,j,domhi(3)+1,1:3) = 2.d0*vel(i,j,domhi(3)+1,1:3) - vel(i,j,domhi(3),1:3)
               end select
            end if
