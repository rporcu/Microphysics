The MFIX-Exa executable reads run-time information from an "inputs" file (which you put on the command line) 
...
Parameter & Definition & Acceptable Values &Default\\
\hline
{\bf mfix.lo\_bc} & boundary type of each low face  & 0,1,2,3,4,5 & must be set \\
{\bf mfix.hi\_bc} & boundary type of each high face & 0,1,2,3,4,5 & must be set \\
\hline
\end{tabular}
\label{Table:BC}
\end{center}
...
\noindent Note -- {\bf mfix.lo\_bc} and {\bf mfix.hi\_bc} must be consistent with 
...
\item {\bf mfix.lo\_bc} = 1 4 0 

\item {\bf mfix.hi\_bc} = 2 4 0 
...
Parameter & Definition & Acceptable Values &Default\\
\hline
{\bf mfix.allow\_untagging} & are cells allowed to be "untagged" & 0 or 1 & 0 \\
\hline
\end{tabular}
\label{Table:Tagging}
\end{center}
...
we set {\bf mfix.allow\_untagging} = 1 then the user is allowed to "untag" cells in the
...
{\bf mfix.final\_a}   & if {\bf mfix.use\_comoving = t} and positive value then this is final value of $a$  & Real $> 0$ & -1.0 \\
{\bf mfix.final\_z}   & if {\bf mfix.use\_comoving = t} and positive value then this is final value of $z$  & Real $> 0$ & -1.0 \\
\hline
\end{tabular}
...
a final value of $a$ by setting {\bf mfix.final\_a}, 
or a final value of $z$ by setting {\bf mfix.final\_z}.
...
\section{Time Step}
\begin{itemize}
\item If {\bf mfix.do\_hydro}$ = 1$, then typically 
the code chooses a time step based on the CFL number (dt = cfl * dx / max(u+c) ).

\item If {\bf mfix.do\_hydro}$ = 0$ and we are running with dark matter particles, then we use a time step
...
Parameter & Definition & Acceptable Values &Default\\
\hline
{\bf mfix.cfl}    & CFL number for hydro    & Real $> 0$ and $\leq 1$ & 0.8 \\ 
{\bf particles.cfl} & CFL number for particles& Real $> 0$ and $\leq 1$ & 0.5 \\ 
{\bf mfix.init\_shrink} & factor by which to shrink the initial time step & Real $> 0$ and $\leq 1$ & 1.0 \\ 
{\bf mfix.change\_max} & factor by which the time step can grow in subsequent steps & Real $\geq 1$ & 1.1 \\ 
{\bf mfix.fixed\_dt} &  level 0 time step regardless of cfl or other settings & Real $> 0$ & unused if not set \\
{\bf mfix.initial\_dt} &  initial level 0 time step regardless of other settings & Real $> 0$ & unused if not set \\
{\bf mfix.dt\_cutoff} & time step below which calculation will abort & Real $> 0$ & 0.0 \\
\hline
\end{tabular}
...
\item {\bf mfix.fixed\_dt} = 1.e-4\\
sets the level 0 time step to be 1.e-4 for the entire simulation, 
ignoring the other timestep controls.   Note that if {\bf mfix.init\_shrink} $\neq 1$
then the first time step will in fact be {\bf mfix.init\_shrink} * {\bf mfix.fixed\_dt}.

\item {\bf mfix.initial\_dt} = 1.e-4\\
sets the {\it initial} level 0 time step to be 1.e-4 regardless of 
{\bf mfix.cfl} or {\bf mfix.fixed\_dt}.  The time step can
grow in subsequent steps by a factor of {\bf mfix.change\_max} each step.

\item {\bf mfix.dt\_cutoff} = 1.e-20\\
...
\label{sec:PlotFiles}
The main output from MFIX-Exa is in the form of plotfiles (which are really directories).
...
{\bf mfix.plot\_phiGrav} & Should we plot the gravitational potential & 0 or 1 & 0 \\
...
All the options for {\bf amr.derive\_plot\_vars} are kept in \texttt{derive\_lst} in MFIX-Exa\_setup.cpp.  Feel free to look at it and see what's there. 
...
\item {\bf mfix.plot\_phiGrav} is only relevant if 
{\bf mfix.do\_grav} = 1 and {\bf gravity.gravity\_type} = PoissonGrav
...
{\bf mfix.v} & verbosity of MFIX-Exa.cpp & 0 or 1 & 0 \\
...
{\bf mfix.do\_special\_tagging} & & 0 or 1 & 1 \\
\hline
\end{tabular}
\end{center}
\end{scriptsize}
...
\item {\bf mfix.do\_special\_tagging} = 1 allows the user to set a special flag based on
...
\item {\bf mfix.sum\_interval} = 2 \\
if {\bf mfix.sum\_interval} $> 0$ then the code computes and prints certain integral quantities, 
such as total mass, momentum and energy in the domain every {\bf mfix.sum\_interval} level 0 steps. 
...
Parameter & Definition & Acceptable Values &Default\\
\hline
{\bf mfix.do\_grav}  & Include gravity as a forcing term & 0 if false, 1 if true & must be set if USE\_GRAV = TRUE  \\
{\bf gravity.gravity\_type} & if {\bf mfix.do\_grav} = 1, how shall gravity be calculated & CompositeGrav, & \\
...
\item {\bf mfix.do\_grav} = 1 in the inputs file
\end{itemize}
\item {\bf gravity.gravity\_type} is  only relevant if {\bf mfix.do\_grav} = 1 
...
Parameter & Definition & Acceptable Values &Default\\
\hline
{\bf mfix.do\_hydro} & Time-advance the fluid dynamical equations & 0 if false, 1 if true & must be set \\
{\bf mfix.do\_react} & Include reactions                          & 0 if false, 1 if true & must be set \\
{\bf mfix.add\_ext\_src} & Include additional user-specified source term & 0 if false, 1 if true & 0 \\
{\bf mfix.use\_const\_species} & If 1 then read h\_species and he\_species & 0 or 1 & 0 \\
{\bf mfix.h\_species}  & Concentration of H  & 0 $<$ X $<$ 1 & 0 \\
{\bf mfix.he\_species} & Concentration of He & 0 $<$ X $<$ 1 & 0 \\
\hline
\end{tabular}
\label{Table:Physics}
\end{center}
