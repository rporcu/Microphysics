...
The '[*]' can be preceded by a space, as is done in MAESTRO (04/24/18), but
...
The basic function of the script is as follows:
 ...
 5) The parameters that occurred with a [*] are recorded as covered and the
...
def main(cwd=None):
    ...
    for i, file_path in enumerate(file_paths):
        ...
        covered_temp, no_cover_temp, All = list_parameters(file_path, All)

        if i == 0:
            no_cover = no_cover_temp  # Initializing no_cover
        if i == 0:
            covered = covered_temp  # Initializing covered
    ...
    output_coverage(SPEC_FILE, covered, no_cover, covered_Frac, no_cover_Frac)

    covered_no_specific, no_cover_no_specific = remove_specific_params(covered, no_cover, ignore)
    ...
    output_coverage(
        ...
        covered_no_specific,
        no_cover_no_specific,
        covered_no_specificFrac,
        no_cover_no_specificFrac,
        specific=ignore,
    )
...
def build_master(covered_temp, no_cover_temp, covered, no_cover, ignore, ignore_master, all_params):
    # Updates the master list of covered and not covered variables.

    # Handles the set of no_cover parameters for a test
    for no_cov in no_cover_temp:

        if no_cov in ignore:
            ignore.remove(no_cov)
        elif no_cov not in ignore_master:
            ignore.append(no_cov)
            ignore_master.append(no_cov)
        else:
            pass

        if no_cov not in (no_cover or covered):
            no_cover.append(no_cov)
        else:
            pass

        if no_cov not in all_params:
            all_params.append(no_cov)
        else:
            pass

    # Handles the set of covered parameters for a test
    for cov_tmp in covered_temp:

        if cov_tmp in ignore:
            ignore.remove(cov_tmp)
        elif cov_tmp not in ignore_master:
            ignore.append(cov_tmp)
            ignore_master.append(cov_tmp)

        if cov_tmp not in covered:
            # Adds newly covered parameters
            covered.append(cov_tmp)

        if cov_tmp in (no_cover or covered):
            # Removes newly covered parameters
            no_cover.remove(cov_tmp)

        if cov_tmp not in all_params:
            all_params.append(cov_tmp)
...
def get_files():
    ...
    dirs = filter(os.path.isdir, os.listdir(data))
    ...
    for dir_ in dirs:
        for file in os.listdir(dir_):
            ...
            if file.endswith(".tgz"):
                tmp = os.path.join(dir_, file)
                file_name = os.path.join(data, os.path.splitext(file)[0])
                # Extracts the job_info file
                tarfile.open(tmp).extract(os.path.splitext(file)[0] + "/job_info")
                file_name = file_name + "/job_info"

                file_here = os.path.join(dir_, file_name)
...
def output_coverage(output_name, covered, no_cover, covered_frac, _no_cover_frac, specific=()):
    ...
    with open(output_name, mode="w") as coverage:
        ...
        for test in no_cover:
            coverage.write(test + "\n")

        coverage.write("================================================== \n")
        coverage.write("Coverage: {0:5.2f}% \n".format(covered_frac * 100))
...
def clean(file_paths):
    ...
    for file_path in file_paths:
        ...
        os.remove(file_path)
        directory = file_path.replace("/job_info", "")
        os.rmdir(directory)
