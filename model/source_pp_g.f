...
! Cutting the neighbor link between fluid cell and adjacent p_flow_at cell
            if(p_flow_at(iminus(i,j,k),j,k)) A_m(IJK,W) = ZERO
            if(p_flow_at(iplus(i,j,k),j,k)) A_m(IJK,E) = ZERO
            if(p_flow_at(i,jminus(i,j,k),k)) A_m(IJK,S) = ZERO
            if(p_flow_at(i,jplus(i,j,k),k)) A_m(IJK,N) = ZERO
            if(p_flow_at(i,j,kminus(i,j,k))) A_m(IJK,B) = ZERO
            if(p_flow_at(i,j,kplus(i,j,k))) A_m(IJK,T) = ZERO
         ENDIF
          end do
        end do
      end do
