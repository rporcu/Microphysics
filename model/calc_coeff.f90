...
!-----------------------------------------------------------------------
      ...
      IF (DES_EXPLICITLY_COUPLED) CALL CALC_DRAG_DES_EXPLICIT(flag, ep_g, &
         u_g, v_g, w_g, ro_g, mu_g, f_gds, drag_bm, pijk, &
         particle_phase, iglobal_id, particle_state, &
         pvol, des_pos_new, des_vel_new, des_radius,  des_usr_var, pinc)
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      SUBROUTINE CALC_COEFF(flag, pLevel, ro_g, p_g, ep_g, rop_g, u_g, v_g, &
         w_g, mu_g, f_gds, drag_bm, pijk, particle_phase, particle_state, &
         pvol, des_vel_new, des_radius)
