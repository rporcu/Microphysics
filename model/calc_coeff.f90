...
!-----------------------------------------------------------------------
      ...
      ! and exchange rates.
      call calc_coeff(&
          slo, shi, lo, hi, max_pip, flag, &
          2, ro_g, p_g, ep_g, rop_g, &
          u_g, ulo, uhi, v_g, vlo, vhi, w_g, wlo, whi, &
          mu_g, f_gds, drag_bm,  particle_phase, &
          particle_state, pvol, des_pos_new, des_vel_new, des_radius, &
          dx, dy, dz)
...
! Calculate interphase coeffs: (momentum and energy)
      if (des_continuum_coupled .and. .not.des_explicitly_coupled)   &
       call calc_drag_des_2fluid(slo, shi, lo, hi, max_pip, &
         u_g, ulo, uhi, v_g, vlo, vhi, w_g, wlo, whi, &
         ep_g, ro_g, mu_g, f_gds, drag_bm, particle_state,  &
...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      subroutine calc_trd_and_tau(slo,shi,lo,hi,&
         tau_u_g,tau_v_g,tau_w_g,trd_g,ep_g,&
         u_g,ulo,uhi,v_g,vlo,vhi,w_g,wlo,whi,&
      ...
      call calc_tau_u_g (slo,shi,lo,hi,tau_u_g,trd_g,ep_g,&
                         u_g,ulo,uhi,v_g,vlo,vhi,w_g,wlo,whi,lambda_g,mu_g,flag,dx,dy,dz)
      call calc_tau_v_g (slo,shi,lo,hi,tau_v_g,trd_g,ep_g,&
                         u_g,ulo,uhi,v_g,vlo,vhi,w_g,wlo,whi,lambda_g,mu_g,flag,dx,dy,dz)
      call calc_tau_w_g (slo,shi,lo,hi,tau_w_g,trd_g,ep_g,&
                         u_g,ulo,uhi,v_g,vlo,vhi,w_g,wlo,whi,lambda_g,mu_g,flag,dx,dy,dz)
