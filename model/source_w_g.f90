...
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
   subroutine source_w_g(slo, shi, lo, hi, A_m, b_m, &
        dt, p_g, ep_g, ro_g, rop_g, rop_go, w_go, &
        tau_w_g, dx, dy, dz)
...
!                                                                      !
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
      subroutine source_w_g_bc(slo, shi, A_m, b_m, &
         flag, dx, dy)
      ...
      if (nlft .gt. 0) then
         ...
         do k=slo(3),shi(3)
            do j=slo(2),shi(2)
               ...
               if(bc_i_type(1,j,k) == NSW_) then
                  A_m(i,j,k,0) = A_m(i,j,k,0)-A_m(i,j,k,w)
                  A_m(i,j,k,w) = zero

               else if(bc_i_type(1,j,k) == FSW_) then
                  A_m(i,j,k,0) = A_m(i,j,k,0)+A_m(i,j,k,w)
                  A_m(i,j,k,w) = zero

               else if(bc_i_type(1,j,k) == PSW_) then
...
! --- TOP FLUID ----------------------------------------------------------->
      ...
      if (ndwn .gt. 0) then
         ...
         do j=slo(2),shi(2)
            do i=slo(1),shi(1)
               bcv = bc_k_ptr(1,i,j)
               if(bc_k_type(1,i,j) == MINF_ .or. &
                  bc_k_type(1,i,j) == MOUT_) then

                  A_m(i,j,k-1,:) =  zero
                  A_m(i,j,k-1,0) = -one
                  b_m(i,j,k-1) = -bc_w_g(bcv)
               endif
            end do
         end do
      endif
         ...
         do j=slo(2),shi(2)
            do i=slo(1),shi(1)
               ...
               if(bc_k_type(2,i,j) == PINF_ .or. &
                  bc_k_type(2,i,j) == POUT_) then
                  A_m(i,j,k,0) = A_m(i,j,k,0)+A_m(i,j,k,t)
                  A_m(i,j,k,t) = zero

               else if(bc_k_type(2,i,j) == MINF_ .or. &
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
      SUBROUTINE POINT_SOURCE_W_G(slo, shi, B_M, flag, dx, dy, dz)
